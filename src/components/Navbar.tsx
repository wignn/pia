    ...
    <header className="site-nav">
      ...
      <div className="nav-center-stacked">
        ...
        <div className="nav-socials">
          <a href="https://discord.gg" target="_blank" rel="noreferrer" aria-label="Discord Bot">
          ...
          <a href="https://github.com/wignn/atlsd" target="_blank" rel="noreferrer" aria-label="GitHub">
      ...
      <div className="nav-actions">
        <div className="products-wrap">
          <button className="nav-link products-trigger" onClick={() => setOpen((value) => !value)} aria-expanded={open}>PRODUCTS <span className="chevron" aria-hidden="true">▾</span></button>
          <AnimatePresence>
            {open && (
              <motion.div className="products-menu" initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>
                {products.map((product) => <a href="#features" key={product} onClick={() => setOpen(false)}>{product}</a>)}
              </motion.div>
            )}
          </AnimatePresence>
        </div>
